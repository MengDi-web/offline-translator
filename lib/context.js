...
function zhOverlap(sense, translated) {
  ...
  let hits = 0;
  const cs = String(sense);
  for (let n = 1; n <= 2; n++) {
    for (let i = 0; i + n <= cs.length; i++) if (seen.has(cs.slice(i, i + n))) hits++;
  }
  return hits;
}
...
function enOverlap(sense, translated) {
  ...
  let hits = 0;
  for (const w of String(sense).toLowerCase().split(/[^a-z']+/).filter(Boolean)) {
...
async function contextTranslate(payload, nmtTranslate) {
  const selection = normalizeWrapped(String(payload.selection || '').trim());  // PDF 断行合并
